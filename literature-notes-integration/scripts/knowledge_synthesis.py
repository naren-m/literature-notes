...
class KnowledgeSynthesizer:
    ...
    def generate_daily_synthesis(self, num_notes: int = 5) -> Dict:
        ...
        for seed in seed_notes:
            ...
            if len(group['notes']) > 1:  # Only include groups with connections
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "daily":
            ...
            print(f"\nDaily synthesis generated with {len(synthesis['groups'])} knowledge clusters")
            ...
            print(f"Shared concepts: {', '.join(insights['shared_concepts'][:5])}")
